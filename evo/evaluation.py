...
def create_world(record_history, seed, ind, r_label, p_label):
    ...
    world[math.floor(body_length * 1.5) + int(aperture_size / 2) + 1, body_length * 2: body_length * 3 + 1, :] = immovable
    world[math.floor(body_length * 1.5) - int(aperture_size / 2): math.floor(body_length * 1.5) + int(aperture_size / 2) + 1,
    ...
    if record_history:  # just save history, don't assign fitness
        ...
        for r_num, r_label in enumerate(['a', 'b', 'c']):
            for p_num, p_label in enumerate(["passable", "impassable"]):
                ...
                sub.call(
                    "./voxcraft-sim -i data{0} > {0}_id{1}_fit{2}.hist".format(str(seed) + str(r_label) + str(p_label),
                                                                               pop[0].id,
